...
void process_instruction(){
   ...
   *       -Update NEXT_LATCHES
   */
   int instr, opcode, dr, sr1, sr2, imm5, baseR, offset6, offset9, offset11, address, temp, trapvect8;  
   ...
   switch(opcode){
    case 0: // BR
      ...
      offset9 = instr & 0x1FF;
      if (offset9 & 0x100){
        offset9 |= 0xFE00;} // sign extend if the 9th bit is 1
      if ((n && CURRENT_LATCHES.N) || (z && CURRENT_LATCHES.Z) || (p && CURRENT_LATCHES.P)) {
        NEXT_LATCHES.PC = NEXT_LATCHES.PC + (offset9 << 1);}
      }
      break;
    ...
    case 2: // LDB
        {
            ...
            baseR = (instr >> 6) & 0x7;
            offset6 = instr & 0x3F;
            if (offset6 & 0x20) offset6 |= 0xFFFFFFC0; // sign extend
            address = CURRENT_LATCHES.REGS[baseR] + offset6;
            temp = MEMORY[address>>1][address & 0x1];
    ...
    case 6: //LDW
        {
            ...
            offset6 = instr & 0x3F;
            if (offset6 & 0x20) offset6 |= 0xFFFFFFC0; // sign extend
            address = CURRENT_LATCHES.REGS[baseR] + (offset6 << 1); // left shift 1 for word address
            temp = (MEMORY[address>>1][1] << 8) | MEMORY[address>>1][0];
            NEXT_LATCHES.REGS[dr] = Low16bits(temp);
    ...
    case 7: // STW
        {
            ...
            MEMORY[address>>1][1] = (CURRENT_LATCHES.REGS[sr1] >> 8) & 0xFF;
        }
        break;
    ...
    case 4: // JSR/JSRR
        {
            if ((instr >> 11) & 0x1) { // JSR
                ...
                if (offset11 & 0x400) offset11 |= 0xF800; // sign extend
    ...
    case 5: // AND
        {
            ...
            if ((instr >> 5) & 0x1) { // immediate mode
                imm5 = instr & 0x1F;
                if (imm5 & 0x10) imm5 |= 0xFFFFFFE0; // sign extend
                NEXT_LATCHES.REGS[dr] = Low16bits(CURRENT_LATCHES.REGS[sr1] & imm5);
            } else { // register mode
                sr2 = instr & 0x7;
                NEXT_LATCHES.REGS[dr] = Low16bits(CURRENT_LATCHES.REGS[sr1] & CURRENT_LATCHES.REGS[sr2]);
    ...
    case 8: // RTI
        // LAB doc says don't need to implement
        exit(4);
        break;
    case 9: // XOR or NOT
        {
    ...
    case 14: //LEA
        {
            dr = (instr >> 9) & 0x7;
            offset9 = instr & 0x1FF;
            if (offset9 & 0x100) offset9 |= 0xFE00; // sign extend
            NEXT_LATCHES.REGS[dr] = Low16bits(NEXT_LATCHES.PC + (offset9 << 1));
        }
        break;
