...
int GetMARMUX(int *x)        { return(x[MARMUX0]); }
...
int GetGATE_SPMATH(int *x)   { return(x[GATE_SPMATH]); }
...
int GetMARMUX1(int *x)       { return(x[MARMUX1]); }
int GetCONDTRAN(int *x)      { return(x[CONDTRAN]); }
int GetCONDRET(int *x)       { return(x[CONDRET]); }
...
void eval_micro_sequencer() {
   ...
   */
    ...
    int condtran = GetCONDTRAN(CURRENT_LATCHES.MICROINSTRUCTION);
    int condret = GetCONDRET(CURRENT_LATCHES.MICROINSTRUCTION);

    // Microsequencer w/ VMEM logic (muxes in diagram)
    if (condtran == 1) {
        NEXT_LATCHES.SS = j;
        NEXT_LATCHES.STATE_NUMBER = 57;
    } else if (condret == 1) {
        NEXT_LATCHES.STATE_NUMBER = CURRENT_LATCHES.SS;
    } else if (ird == 1) {
        ...
        if (cond == 0) {
            ...
            NEXT_LATCHES.STATE_NUMBER = j | (CURRENT_LATCHES.READY << 1);
        } else if (cond == 2) { // BEN
            NEXT_LATCHES.STATE_NUMBER = j | (CURRENT_LATCHES.BEN << 2);
        } else if (cond == 3) { // IR 11
            NEXT_LATCHES.STATE_NUMBER = j | ((CURRENT_LATCHES.IR >> 11) & 0x0001);
        } else if(cond == 4) {
            NEXT_LATCHES.STATE_NUMBER = j | ((CURRENT_LATCHES.PSR_15) << 3);
        }
    }
    ...
    if (ird == 1 && (NEXT_LATCHES.STATE_NUMBER == 10 || NEXT_LATCHES.STATE_NUMBER == 11)) {
        NEXT_LATCHES.EXC = 1;
        NEXT_LATCHES.VECT = 0x05;
    }

    // State 18 check for int
    ...
    // Protection & Page Fault check in state 58
    if (CURRENT_LATCHES.STATE_NUMBER == 58) {
        int pte = CURRENT_LATCHES.MDR;
        int pbit = (pte >> 3) & 0x01;
        int vbit = (pte >> 2) & 0x01;
        int opcode = (CURRENT_LATCHES.IR >> 12) & 0x000F;

        if (pbit == 0 && CURRENT_LATCHES.PSR_15 == 1 && opcode != 0xF) {
            NEXT_LATCHES.EXC = 1;
...
void cycle_memory() {
   ...
   */
    ...
    if (mio_en == 1) {
        mem_count ++;
        if(mem_count == MEM_CYCLES - 1) {
            NEXT_LATCHES.READY = 1;
        } else if (mem_count == MEM_CYCLES) {
            int addr = CURRENT_LATCHES.MAR >> 1; // Make a word addressable address
            int mar_0 = CURRENT_LATCHES.MAR & 0x0001; // Even or odd ending address for byte

            if (rw == 0) { // Read
                int lowB = MEMORY[addr][0];
                int highB = MEMORY[addr][1] << 8;
                NEXT_LATCHES.MDR = Low16bits(lowB | highB);
...
void eval_bus_drivers() {
   ...
   */    
    ...
    int gate_spmath = GetGATE_SPMATH(CURRENT_LATCHES.MICROINSTRUCTION);
    int gate_sp = GetGATE_SP(CURRENT_LATCHES.MICROINSTRUCTION);
    int gate_vect = GetGATE_VECT(CURRENT_LATCHES.MICROINSTRUCTION);
    int gate_pte = GetGATE_PTE(CURRENT_LATCHES.MICROINSTRUCTION);

    if (gate_marmux == 1){
        bus_marmux = eval_MARMUX();
    } if (gate_pc == 1) {
    ...
    // Lab 4 logic
        ...
        bus_spmath = eval_SPMATH();
    } if (gate_psr == 1) {
        bus_psr = (CURRENT_LATCHES.PSR_15 << 15) | (CURRENT_LATCHES.N << 2) | (CURRENT_LATCHES.Z << 1) | CURRENT_LATCHES.P;
    } if (gate_vect == 1) {
        bus_vect = Low16bits(0x0200 + (CURRENT_LATCHES.VECT << 1));
    } if (gate_pte == 1) {
        bus_pte = eval_PTE();
    }
...
void latch_datapath_values() {
   ...
   */       
    ...
    int drmux1 = GetDRMUX1(CURRENT_LATCHES.MICROINSTRUCTION);
    int ld_va = GetLD_VA(CURRENT_LATCHES.MICROINSTRUCTION);
    ...
    if (ld_mar == 1) {
        ...
        int dr;
        if (drmux1 == 1) { // Lab 4 check new drmux bit to select R6
            dr = 6;
        }else if (drmux == 0) {
    ...
    // Lab 4 PSR logic so loading PSR from MDR isn't messed up
    } if (GetLD_PSR(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
        NEXT_LATCHES.PSR_15 = (BUS >> 15) & 0x01;
    } if (ld_cc == 1) {
        if (GetLD_PSR(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
            NEXT_LATCHES.N = (BUS >> 2) & 0x01;
            NEXT_LATCHES.Z = (BUS >> 1) & 0x01;
...
int eval_MARMUX() {
    int marmux0 = GetMARMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    int marmux1 = GetMARMUX1(CURRENT_LATCHES.MICROINSTRUCTION);
    int marmux = (marmux1 << 1) | marmux0;
    int gate_marmux = GetGATE_MARMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    ...
    if (marmux == 0 && gate_marmux == 1) { // only in TRAP
        ...
        return Low16bits(ptbr | vpn << 1);

    } else if (marmux == 3) { // Phys Addr
        int pfn = CURRENT_LATCHES.MDR & 0x3E00;
        int offset = CURRENT_LATCHES.VA & 0x01FF;
        return Low16bits(pfn | offset);
    }
...
int eval_ALU() {
    ...
    // Val 1
    int sr1_reg;
    if (sr1mux1 == 1) { // Lab 4 using new sr1mux bit to select R6
        sr1_reg = 6;
    } else if (sr1 == 0) { // IR[11:9]
        sr1_reg = (CURRENT_LATCHES.IR >> 9) & 0x07;
    } else { // IR [8:6]
        sr1_reg = (CURRENT_LATCHES.IR >> 6) & 0x07;
    }
    val1 = CURRENT_LATCHES.REGS[sr1_reg];

    // Val 2
    int steering  = (CURRENT_LATCHES.IR >> 5) & 0x01;
    if (steering == 0) { // Reg
        int sr2_reg = CURRENT_LATCHES.IR & 0x07;
        val2 = CURRENT_LATCHES.REGS[sr2_reg];
    } else {
        val2 = signExtend((CURRENT_LATCHES.IR & 0x1F), 5);
    }
...
int eval_SHF() {
    int sr = (CURRENT_LATCHES.IR >> 6) & 0x07;
    int val = CURRENT_LATCHES.REGS[sr];
    int amt = CURRENT_LATCHES.IR & 0x0F;
    int a_d = (CURRENT_LATCHES.IR >> 4) & 0x03;

    if (a_d == 0) {
        return Low16bits(val << amt);
    } else if (a_d == 1) {
        return Low16bits(((unsigned int)(val & 0xFFFF)) >> amt);
    } else if (a_d == 3) {
        return Low16bits(signExtend(val, 16) >> amt);
    }
...
int eval_MDR() {
    ...
    if (datasize == 1) { // Word
        return Low16bits(data);
    } else { // Byte
        int mar_0 = CURRENT_LATCHES.MAR & 0x01; // Even or odd addr
        if (mar_0 == 0) {
            return Low16bits(signExtend((data & 0xFF), 8));
        } else {
            return Low16bits(signExtend(((data >> 8) & 0xFF), 8));
        }
    }
}

int eval_SPMATH() {
    int stackmux = GetSTACKMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    if (stackmux == 0) {
        return Low16bits(CURRENT_LATCHES.REGS[6] + 2); // Pop
    } else {
        return Low16bits(CURRENT_LATCHES.REGS[6] - 2); // Push
    }
}
...
int eval_SP() {
    ...
    if (spmux == 0) {
        return CURRENT_LATCHES.SSP; // 0 for SSP
    } else {
        return CURRENT_LATCHES.USP; // 1 for USP
    }
}
